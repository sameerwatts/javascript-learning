const DUMMY_STRING = `Skip to main content Skip to search
...
function Person() {
  var self = this; // Some choose  instead of .
                   // Choose one and be consistent.
  self.age = 0;

  setInterval(function growUp() {
    // The callback refers to the variable of which
...
const dummyArr = DUMMY_STRING.split('');
// remove all empty spaces
const cleanArrayWithoutSpaces = dummyArr
  .map((singleChar) => singleChar.toLocaleLowerCase())
  .filter((char) => char !== ' ' && !!char.match(/^[a-zA-Z1-9]*$/));

const reducedObj = cleanArrayWithoutSpaces.reduce(
  (generatedObj, currentItem) => {
    if (!generatedObj[currentItem]) {
      generatedObj[currentItem] = 1;
      return generatedObj;
    }
    generatedObj[currentItem] += 1;
    return generatedObj;
